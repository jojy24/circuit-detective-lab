    ...
    <div className="min-h-screen bg-cream text-foreground antialiased">
      ...
      <main className="mx-auto max-w-7xl px-6 pb-24 pt-10 sm:px-8">
        <section className="mb-14 grid gap-8 md:grid-cols-[minmax(0,1.3fr)_minmax(0,1fr)] md:items-end">
          <div>
            <div className="font-tech text-[10px] uppercase tracking-[0.32em] text-secondary">chapter 02 · the desk</div>
            <h1 className="mt-4 font-editorial text-5xl leading-[1.02] text-foreground sm:text-6xl">Case files, <span className="italic text-primary">{profile ? profile.name.split(" ")[0] : "engineer"}</span>.</h1>
            <p className="mt-4 max-w-lg font-hand text-2xl leading-snug text-ink">"five sealed folders on the desk. one is open. the rest unlock as you prove yourself."</p>
          </div>
          <div className="rounded-sm bg-card p-6 shadow-[var(--shadow-paper)]">
            <div className="grid grid-cols-3 gap-4">
              <Stat label="solved" value={`${solvedCount}/${CASES.length}`} />
              <Stat label="total xp" value={`${totalXp(progress)}`} sub={`of ${totalXpAvailable}`} />
            ...
            {allDone && (
              <button onClick={() => navigate({ to: "/certificate" })} className="mt-4 inline-flex w-full items-center justify-center gap-2 rounded-full bg-primary px-4 py-2.5 font-tech text-[11px] uppercase tracking-[0.22em] text-primary-foreground"><Award className="h-4 w-4" /> Claim certificate</button>
            )}
          </div>
        </section>
        ...
        {profile && (
          <div className="mt-12 flex items-center justify-between border-t border-foreground/15 pt-6 font-tech text-[10px] uppercase tracking-[0.24em] text-muted-foreground">
            <span>signed in as <span className="text-foreground">{profile.name}</span> · {profile.level}</span>
            <button onClick={() => setProfile(null)} className="hover:text-primary">reset profile</button>
          </div>
        )}
      </main>
